module FcsvrConnector
  ...
  class FcsClient
    ...
    def getmd(entity_addr)
      command = @@fcsvr_client_cmd + " getmd --xml " + entity_addr
      output = IO.popen(command) do |pipe|
        pipe.read
      end
      xml_doc = REXML::Document.new output
      metadata = Hash.new
      metadata["cmd_executed"] = command
      metadata["raw_xml_response"] = output
    ...
    def search(search_xml_builder, container_addr)
      command = @@echo_cmd + " '" + search_xml_builder.to_xml + "' | " + @@fcsvr_client_cmd + " search --xmlcrit --xml " + container_addr
      output = IO.popen(command) do |pipe|
        pipe.read
      end
      xml_doc = REXML::Document.new output
